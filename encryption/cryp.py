    ...
    def Encode(self,image,noise=True,noise_level=100,shuffling_seed=None):
        ...
        if len(img.shape) != 3:
            ...
            for c in range(img.shape[2]):
                ...
                blocks = self.M2block(img[:,:,c], noise_level, noise)
...
class AES:
    ...
    def __init__(self, image_size=(32, 32), block_size=(4, 4), One_cipher=True, Shuffle=False):
        ...
        # Update img_size to the post-scale dimensions so padding/block
        # counts are computed against the image that Encode will actually work on.
        if self.scale != [1, 1]:
            if len(image_size) == 3:
                self.img_size = (image_size[0] * self.scale[0], image_size[1] * self.scale[1], image_size[2])
            else:
                self.img_size = (image_size[0] * self.scale[0], image_size[1] * self.scale[1])
    ...
    def scaleup(self, img):
        ...
        assert (self.scale != [1, 1] and img.shape[0:2] == self.orig_img_size[0:2])

        if len(img.shape) == 3:
            img1 = np.ones((img.shape[0] * self.scale[0], img.shape[1] * self.scale[1], img.shape[2]))
        else:
            img1 = np.ones((img.shape[0] * self.scale[0], img.shape[1] * self.scale[1]))
    ...
    def scaledown(self, img):

        '''Merge neighboring pixels to return to the pre-scale resolution.'''

        assert self.scale != [1, 1]

        h0, w0 = self.orig_img_size[0], self.orig_img_size[1]
        sy, sx = self.scale[0], self.scale[1]

        # Keep only whole scale windows in case padding introduced extra pixels.
        h = min(img.shape[0], h0 * sy)
        w = min(img.shape[1], w0 * sx)
        h = (h // sy) * sy
        w = (w // sx) * sx
        img = img[:h, :w] if len(img.shape) == 2 else img[:h, :w, :]

        if len(img.shape) == 3:
            img_small = img.reshape(h // sy, sy, w // sx, sx, img.shape[2]).mean(axis=(1, 3))
        else:
            img_small = img.reshape(h // sy, sy, w // sx, sx).mean(axis=(1, 3))

        return np.clip(img_small, -128, 127).astype(np.byte)
    ...
    def Encode(self, img, noise = False, noise_level = 1, shuffling_seed = None):
        ...
        # For block_size < 16, we first scale up to a 16-byte unit and then
        # scale back down to the original resolution.
        if self.scale != [1, 1]:
            img2 = self.scaledown(img2)
